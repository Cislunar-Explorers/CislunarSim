...
class CislunarSim:
    ...
    def __init__(self, config: Config, fsw_run) -> None:
        self._config = config
        # wouldn't need for event-based
    ...
    def step(self) -> PropagatedOutput:
        ...
        self.state_time, self.observed_state = current_event.evaluate(
            self.state_time)
    ...
    def should_stop(self) -> bool:
        ...
        if (self.state_time.time - self._config.init_cond.time) > 6.312e7:
            ...
            log.debug(
                f"Elapsed time = {int(self.state_time.time - self._config.init_cond.time)}s > 6.312e7")
            return True
        ...
        if r_e > 5 * EARTH_SOI:
            log.error(
                "Stopping sim because craft in Heliocentric orbit (way outside of Earth's SOI)")
